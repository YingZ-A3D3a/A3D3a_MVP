...
def depmap_validation_detailed(dir0,save_dir,wes_dir,t_types,mt_types,max_n = 600,topN = 100,
                               types = ['MVP','Top altered'],
    ...
    for t_type in t_types:
        ...
        fq_dir = os.path.join(wes_dir, t_type, 'TCGA_'+t_type+'_wes_freq_long.csv')
        ...
        freq_list = freq_q
        make_plots(in_depmap_list,t_type,freq_list,types = ['MVP','Top altered DNA'],save_dir1 = save_dir1,topN = topN,colors = colors)
        ...
        make_plots(in_depmap_list_rm_ce,t_type,freq_list,types = ['MVP','Top altered DNA'],save_dir1 = save_dir1,topN = topN,colors = colors,ftype = 'remove_common_essential')
    ...
    df_all.columns = ['MVP','Top altered DNA']
    make_boxplot(df_all,colors = colors,types = ['MVP','Top altered DNA'],topN = topN,save_dir1 = save_dir1,ftype = 'AUC_including_common_essentials')
    ...
    df_all = pd.DataFrame(area_mat)
    df_all.columns = ['MVP','Top altered DNA']
    make_boxplot(df_all,colors = colors,types = ['MVP','Top altered DNA'],topN = topN,save_dir1 = save_dir1,ftype = 'AUC_excluding_common_essentials')
    ...
    df_all.columns = ['MVP','Top altered DNA']
    make_boxplot(df_all,colors = colors,types = ['MVP','Top altered DNA'],topN = topN,save_dir1 = save_dir1, ylabel = 'Count of genetic dependencies',ftype = 'including_common_essentials')
    ...
    df_all = pd.DataFrame(c_mat)
    df_all.columns = ['MVP','Top altered DNA']
    make_boxplot(df_all,colors = colors,types = ['MVP','Top altered DNA'],topN = topN,save_dir1 = save_dir1, ylabel = 'Count of genetic dependencies',ftype = 'excluding_common_essentials')

################# compared with top altered genes in RNA, measured by |log2 Fold Change|
    df_rnk = df_rnk.sort_values(by = 'mean_rank')
    select_md = df_rnk.index[0]
    in_depmap_list_all = []
    in_depmap_list_all_rm_ce = []
    colors = ['#00A087FF','#FB6080']
    ...
    for t_type in t_types:
        ...
        fq_dir = os.path.join(save_dir, t_type, 'DEG_all_combatseq_TCGA_primary_vs_GTEx+NAT_022123.csv')
        df_q = pd.read_csv(fq_dir)
        df_q = df_q.loc[df_q.gene.isin(pcg),]
        df_q = df_q.loc[df_q.FDR_BH<0.05,]
        df_q = df_q.sort_values(by = 'abs_log2_fc', ascending = False)
        df_q = df_q.reset_index(drop = True)
        df_q = df_q.iloc[:max_n,]
        abs_log2_q = df_q.abs_log2_fc.values.tolist()    
        ...
        in_depmap_list = [in_depmap,in_depmap_q]
        abs_log2_list = abs_log2_q
        make_plots(in_depmap_list,t_type,freq_list = abs_log2_list,types = ['MVP','Top altered RNA (FC)'],
                topN = topN,colors = colors,save_dir1=save_dir1,
                ylabel_right = '|log2 Fold Change|')
        in_depmap_list_all.append(in_depmap_list)
        ...
        in_depmap_list_rm_ce = [in_depmap_rm_ce,in_depmap_q_rm_ce]
        make_plots(in_depmap_list_rm_ce,t_type,freq_list = abs_log2_list,types = ['MVP','Top altered RNA (FC)'],
                topN = topN,colors = colors,save_dir1=save_dir1,
                ftype = 'remove_common_essential',
                ylabel_right = '|log2 Fold Change|')
        in_depmap_list_all_rm_ce.append(in_depmap_list_rm_ce)

    ###  make boxplot, including common essentials
    types = ['MVP','Top altered RNA (FC)']
    # colors = ['#00A087FF','#7E6148FF']
    colors = ['#91D1C2FF','#FB6080']
    topN = 600

    area_list = []
    for item in range(len(in_depmap_list_all)):
        in_depmap_list = in_depmap_list_all[item]
        a = area_between_curves(in_depmap_list,topN)
        area_list.append(a)
    area_mat = np.array(area_list)
    df_all = pd.DataFrame(area_mat)
    df_all.columns = types
    make_boxplot(df_all,colors,types,topN,save_dir1=save_dir1,ftype = 'including_common_essentials')

    ### make boxplot, excluding common essentials
    area_list = []
    for item in range(len(in_depmap_list_all_rm_ce)):
        in_depmap_list = in_depmap_list_all_rm_ce[item]
        a = area_between_curves(in_depmap_list,topN)
    ...
    df_all = pd.DataFrame(area_mat)
    df_all.columns = types
    make_boxplot(df_all,colors,types,topN,save_dir1=save_dir1,ftype = 'excluding_common_essentials')

    #################### compared with top altered genes in RNA, measured by altered rate
    df_rnk = df_rnk.sort_values(by = 'mean_rank')
    select_md = df_rnk.index[0]
    in_depmap_list_all = []
    in_depmap_list_all_rm_ce = []
    colors = ['#00A087FF','#FF007F']
    ...
    for t_type in t_types:
        ...
        fq_dir = os.path.join(save_dir, t_type, 'TCGA_'+t_type+'_rna_altered_freq_all_genes.csv')
        df_q = pd.read_csv(fq_dir)
        df_q = df_q.loc[df_q.Gene.isin(pcg),]
        df_q = df_q.sort_values(by = 'Freq', ascending = False)
        df_q = df_q.reset_index(drop = True)
        df_q = df_q.iloc[:max_n,]
        Freq = df_q.Freq.values.tolist()    
        ...
        in_depmap_list = [in_depmap,in_depmap_q]
        Freq_list = Freq
        make_plots(in_depmap_list,t_type,freq_list = Freq_list,
                types = ['MVP','Top altered RNA'],save_dir1=save_dir1,
                topN = topN,colors = colors)
        in_depmap_list_all.append(in_depmap_list)
        ...
        in_depmap_list_rm_ce = [in_depmap_rm_ce,in_depmap_q_rm_ce]
        make_plots(in_depmap_list_rm_ce,t_type,freq_list = Freq_list,types = ['MVP','Top altered RNA'],save_dir1=save_dir1,
                topN = topN,colors = colors,ftype = 'remove_common_essential')
        in_depmap_list_all_rm_ce.append(in_depmap_list_rm_ce)

    ### make boxplot, including common essentials
    types = ['MVP','Top altered RNA']
    # colors = ['#00A087FF','#7E6148FF']
    colors = ['#91D1C2FF','#FB6080']
