...
def run_pipeline(gm,genex,s_list,score_ini,alpha0,modelx):  # modelx can be pr or ppr
    ...
    print(f"model: {modelx}, alpha: {alpha0}, new network size: {outs[0]}, neighbors of {genex}:{outs[1]}, rank of {genex}:{outs[2]}")
    return outs[3]
...
def run_pipeline_unequal(gm,genex,s_list,score_ini,alpha,Wm=0.6,modelx='Markov'):
    ...
    print(f"model:{modelx},new network size: {outs[0]}, neighbors of {genex}:{outs[1]}, rank of {genex}:{outs[2]}")
    return outs[3]
