...
# These codes try different parameters for Markov Chain models
####################################################################################
...
####################################################################################
def tuning_pgm(save_dir,t_types,to_remove,thre,fn_set = np.arange(50,650,100)):
    """
    ...
    ###### parameters ######
    ### try different numbers of first neighbors

    ### run pgm model for each cancer type
    def pgm_all_cancers(fn_set,Wms,alphas,save_dir,to_remove):
        for t_type in t_types:
            # seed genes
            save_dir1 = os.path.join(save_dir,t_type,'elastic net model')
            save_dir2 = os.path.join(save_dir,t_type,'markov chain models')
            os.makedirs(save_dir2, exist_ok = True)

            seed_score = os.path.join(save_dir1,'wes+rna_classifier_022123.csv') # seed genes initial score
            ...
            # first neighbors
            fn_dir = os.path.join(save_dir1,'first_neighbors_and_p-value_wes+rna_022123.csv') # first neighbors
                    ...
                    for alpha in alphas:
                        ...
                        for i in final_prob_markov.genes.values:
                            if i not in g_score.index:
                                ...
                                if (g_score.loc[i,'wes']==1)&(g_score.loc[i,'rna']==1):
                                    source.append('WES & RNA')
                                else:
                                    if g_score.loc[i,'wes']==1:
                                        source.append('WES/SV')
                                    if g_score.loc[i,'rna']==1:
                                        source.append('RNA')
                        final_prob_markov['source'] = source        

                        ## save final rank and probability
                        filenm = os.path.join(save_dir2,'markov_output_wes+rna_fn_'+str(fn_num)+'_Wm_'+str(Wm)+'_alpha_'+str(alpha)+'_022123.csv')
    ...
    Wms = [0.3,0.4,0.5]
    alphas = [0,0.05,0.1]
    pgm_all_cancers(fn_set,Wms,alphas,save_dir,to_remove)
