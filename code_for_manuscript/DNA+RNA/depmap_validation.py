...
####################################################################################
def depmap_validation(dir0,save_dir,t_types,mt_types,topN = 100):
    ...
    compare_top_predicted_and_top_altered(t_types,df_rnk,ge_list_all,num_cell_lines,overlap_ce,dir0,save_dir,fig_dir,mat_dir,topN)
...
def compare_top_predicted_and_top_altered(t_types,df_rnk,ge_list_all,num_cell_lines,overlap_ce,dir0,save_dir,fig_dir,mat_dir,topN):
    ...
    for t_type in t_types:
        ...
        fq_dir = os.path.join(save_dir, t_type, 'elastic net model', 'wes+rna_classifier_'+date+'.csv')
    ...
    labels, data = [*zip(*overlap_ct.items())]  # 'transpose' items to parallel key, value lists
    ...
    boxpropsl = dict(linestyle='-', linewidth=0.5, facecolor=colors[0], edgecolor='k',alpha = 0.5)
    boxpropsr = dict(linestyle='-', linewidth=0.5, facecolor=colors[1], edgecolor='k',alpha = 0.5)
    ...
    labels, data_fq = [*zip(*overlap_ct_fq.items())]  # 'transpose' items to parallel key, value lists
    ...
    set_box_color(bpl, color1, linewidth0 = 0.5) # colors are from http://colorbrewer2.org/
    set_box_color(bpr, color2, linewidth0 = 0.5)
    labels1 = [i+' ('+str(num_cell_lines[i])+')' for i in labels]
    plt.xticks(np.arange(0, 5*len(labels),5), labels1, fontsize = 6, rotation = 90)
    ...
    ax.spines['bottom'].set_linewidth(0.75)
    ax.spines['left'].set_linewidth(0.75)
    plt.plot([], c=color1, label='MVP')
    plt.plot([], c=color2, label=f'Top altered')
    plt.legend(fontsize = 5.5)
    fnm = os.path.join(fig_dir,f'Percent of depmap essential genes among top {topN} disease network genes including common essentials boxplot.svg')
    plt.ylabel(f'Essential gene count',fontsize = 7)
    plt.tight_layout()
    plt.savefig(fnm,dpi = 450)
    plt.show()
    ...
    fnm = os.path.join(mat_dir,f'top_{topN}_genes_MVP_with_depmap_essential_info_including_common_essentials.csv')
    top_ess_mat.to_csv(fnm,index = False)
    ### check how many essential genes are first neighbors
    N_ess,N_ess_fn,N_ess_fq = check_first_neighors_are_essential(t_types,overlap_all,overlap_all_fq,labels1,select_md,topN,save_dir)
    save_table_essentials(t_types,N_ess,N_ess_fn,select_md,topN,overlap_all,overlap_ce,pcg,save_dir,mat_dir)
    statistical_test(N_ess,N_ess_fq,topN,fig_dir)

def check_first_neighors_are_essential(t_types,overlap_all,overlap_all_fq,labels1,select_md,topN,save_dir):
   ### check how many essential genes are first neighbors
    N_ess = []
    N_ess_fn = []
    N_ess_fq = []
    ...
    plt.yticks(fontsize = 6)
    title = f'Essential gene count'
    # plt.title('Including common essentials')
    plt.ylabel(title,fontsize = 7)
    plt.ylim([0,topN/2+5])
    plt.legend(["seed", "first neighbor",f'top altered'],fontsize = 5)
    plt.tight_layout()
    fnm = os.path.join(save_dir,'figures',title+f' top {topN} including common essentials.svg')
    plt.savefig(fnm,dpi = 300, bbox_inches='tight')
    return N_ess,N_ess_fn,N_ess_fq

def save_table_essentials(t_types,N_ess,N_ess_fn,select_md,topN,overlap_all,overlap_ce,pcg,save_dir,mat_dir):
    ### create a table for comparison depmap essentials between MVP and top altered
    date = '022123'
    tumor_types = []
    ...
    summary_table = pd.DataFrame({'Tumor type':tumor_types,'Method':types,
                                f'depmap essential count':num_depmap_essential,
                                f'percent of depmap essentials (%)':percent_depmap_essential,
                                f'depmap essentials':depmap_essential_genes,
                                f'common essentials':common_essential_genes})
    ### save dataframe
    fnm = os.path.join(mat_dir,f'compare_depmap_essentials_top_{topN}_ranked_and_top_altered.csv')
    summary_table.to_csv(fnm, index = False)
    ### save data frame of number of pooled depmap essential genes for every cancer type
    num_pool_dep_ess = pd.DataFrame({'tumor_type':t_types,
                                    'number_of_depmap_essentials':N_ess,
                                    'number_of_depmap_essentials_that_are_first_neighbors':N_ess_fn})
    fnm = os.path.join(mat_dir,f'number_of_pooled_depmap_essentials_among_top_{topN}.csv')
    num_pool_dep_ess.to_csv(fnm,index = False)

def statistical_test(N_ess,N_ess_fq,topN,fig_dir):
    ### statistical test
    st = ranksums(N_ess,N_ess_fq)
    tmp = pd.DataFrame({'MVP':N_ess,'top altered':N_ess_fq})
    ...
    plt.yticks(fontsize = 6)
    plt.tight_layout()
    fnm = os.path.join(fig_dir,f'compare_MVP_and_top_{topN}_altered_essential_genes_including_common_essentials.svg')
    plt.savefig(fnm,dpi = 300)
