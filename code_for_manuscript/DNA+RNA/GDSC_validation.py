...
def GDSC_validation(dir0,save_dir,wes_dir,t_types,topN = 100):
    ...
    summary_table = compare_IC50(mat_dir,topN,fig_dir)

    ### compare WES+RNA as seed vs. WES as seed
    # load the summary table for WES as seed
    dir_mat1 = os.path.join(wes_dir,'output_data_matrix')
    fnm1 = os.path.join(dir_mat1,f'compare_targets_in_GDSC_top_ranked_MVP_and_top_{topN}_altered.csv')
    summary_table_wes = pd.read_csv(fnm1)
    tumor_types = sorted(set(summary_table['Tumor type']))
    wes_pct = []
    wes_rna_pct = []
    for t_type in tumor_types:
        wes_pct.append(summary_table_wes.loc[(summary_table_wes['Tumor type']==t_type)&(summary_table_wes['Method']=='MVP'),f'GDSC target count with low z-scored LN_IC50 among top {topN} ranked'].values[0])
        wes_rna_pct.append(summary_table.loc[(summary_table['Tumor type']==t_type)&(summary_table['Method']=='MVP'),f'GDSC target count with low z-scored LN_IC50 among top {topN} ranked'].values[0])
    pct_table = pd.DataFrame({'wes as seed':wes_pct,'wes+rna as seed':wes_rna_pct})
    pct_table.index = tumor_types
    plt.rcParams["figure.figsize"] = (9,6)
    pct_table.plot.bar(color={"wes as seed": "#D30B7F", "wes+rna as seed": "#8491B4"},alpha = 0.8)
    fnm = os.path.join(fig_dir,f'compare wes vs wes+rna of GDSC target count with low z-scored LN_IC50 among top {topN} genes barplot.png')
    plt.ylabel(f"GDSC targets among top {topN} with low z-scored LN_IC50")
...
def heatmap_top_ranked_by_MVP(t_types,dir3,fig_dir,output,topN):
    ### topN ranked by MVP
    for t_type in t_types:
        fnm = t_type+'_GDSC1.csv'
        if fnm in os.listdir(dir3):
            ...
            if dfzx.shape[0]>0:
                ...
                fnm = os.path.join(fig_dir,t_type+'_WES+RNA_as_seed_sensitivity in z-score_top'+str(topN)+'_heatmap_z-score_IC50.png')
                plt.tight_layout()
                plt.savefig(fnm,dpi = 256, bbox_inches='tight')

def heatmap_top_altered(t_types,dir3,fig_dir,output1,topN):
    ### topN altered
    for t_type in t_types:
        fnm = t_type+'_GDSC1.csv'
        if fnm in os.listdir(dir3):
            ...
            if dfzx.shape[1]>0:
                if dfzx.shape[1]>1:
                    ...
                    g.ax_heatmap.set_yticklabels(g.ax_heatmap.get_ymajorticklabels(), fontsize = 9)
                    g.ax_heatmap.set_xlabel('cell line of '+t_type, fontsize=18)
                    g.ax_heatmap.set_ylabel('drug and targets (top 20 altered genes)', fontsize = 18)
                else:
                    ...
                    g.set_ylabel(f'drug and targets (top {topN} altered genes)', fontsize = 18)               
                title = f'sensitivity in z-score'
                plt.title(title, fontsize = 14)
                fnm = os.path.join(fig_dir,t_type+'_WES+RNA_as_seed_sensitivity in z-score_top'+str(topN)+'_altered_heatmap_z-score_IC50.png')
                plt.tight_layout()
                plt.savefig(fnm,dpi = 256, bbox_inches='tight')

def load_top_ranked_with_essential_info(mat_dir,topN):
    file_dir = os.path.join(mat_dir,f'top_{topN}_genes_MVP_with_depmap_essential_info_including_common_essentials.csv')
...
def make_violinplot(x,y,t_type,topN,fig_dir):
    ...
    fnm = os.path.join(fig_dir,t_type+'_WES+RNA as seed_GDSC_violinplot_comparison.png')
    plt.ylabel('z-score of IC50',fontsize = 22)
...
def compare_IC50(mat_dir,topN,fig_dir):
    ...
    fnm = os.path.join(fig_dir,f'GDSC target count with low z-scored LN_IC50 barplot.png')
    plt.savefig(fnm,dpi = 256)
