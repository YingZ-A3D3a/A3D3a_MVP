...
def eval_pr_ppr(model0,t_types,save_dir,cgc_dir,fig_dir,mat_dir,topN = 20,fn_nums = np.arange(50,650,100),alphas = np.round(np.arange(0.05,1,0.05),3)):
    ...
    for fn_num in fn_nums:
        for alpha in alphas:
            m_nm.append('pgm_'+model0+'_wes+rna_fn_'+str(fn_num)+'_alpha_'+str(alpha))  # model parameters
    m_df.index = m_nm
    ...
    df_rnk.to_csv(fnm)
    # plot rank of top 15 ranked model parameters
    top_m = 15
...
def eval_diffusion(t_types,save_dir,cgc_dir,fig_dir,mat_dir,topN = 20,fn_nums = np.arange(50,650,100),sigma_2s = np.arange(5,35,5)):
    ...
    for fn_num in fn_nums:
        for sigma_2 in sigma_2s:
            m_nm.append('diffusion_model_wes+rna_fn_'+str(fn_num)+'_raw_sigma2='+str(sigma_2))  # model parameters
    m_df.index = m_nm
    m_df.columns = t_types
    iou_df.index = m_nm
    iou_df.columns = t_types
...
def model_compare(t_types,df_rnk_MC,df_rnk_pr,df_rnk_ppr,df_rnk_raw,cgc_dir,save_dir,fig_dir,topN):
### compare Markov model, pr, ppr, and diffusion models
    ...
    for t_type in t_types:
        ...
        s_md = 'markov_output_wes+rna_'+select_md_MC+'_022123.csv'
        df = pd.read_csv(os.path.join(md_dir,s_md))    
        # compute the overlap between top ranked genes and tier 1 genes
        top_g = df.iloc[:topN,0].values.tolist()
        overlap = len(set(gt).intersection(top_g))
        iou = len(set(gt).intersection(top_g))/len(set(gt).union(top_g))
        iou_df.loc[t_type,'MC'] = iou        
    ...
    fnm = os.path.join(fig_dir,f'top {topN} compare_different_graphical_models_violinplot_optimal.png')
    plt.savefig(fnm,dpi = 300)
    # statistical test
    a = iou_df.iloc[:,0]
    statistic = []
