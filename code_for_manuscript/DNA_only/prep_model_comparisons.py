...
def prep_model_comparisons(save_dir,t_types,fn_min, fn_max, fn_step,
    ...
    """ 
    Save nodes and edges for the largest connected graph for Markov model with different fn, save nodes and edges
    Run pagerank and personalized pagerank models, with different alpha
    """

    fn_nums = np.arange(fn_min,fn_max+fn_step,fn_step)
    models = ['ppr','pr']
    alpha1s = np.round(np.arange(alpha_min,alpha_max+alpha_step,alpha_step),3)
    gm = ami.adj_mat()
        ...
        for fn_num in fn_nums:           
            ...
            for model in models:
                for alpha1 in alpha1s:
                    ...
                    filenm = os.path.join(om_dir,'pgm_'+model+'_wes_fn_'+str(fn_num)+'_alpha_'+str(alpha1)+'_022123.csv')
