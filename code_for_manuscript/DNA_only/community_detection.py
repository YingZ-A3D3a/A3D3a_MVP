...
####################################################################################
def community_detection_pipeline(dir0,save_dir,t_types,
            resolution = 0.7, seed0 = 12345,plot_topN = 60):
...
"""
def plot_the_top_ranked_disease_network(df,gm,info_df,dep_df,gdsc_df,t_type,t_subtype,fig_dir,topN=20,top_k=60,
    ...
    if len(nodes_1)+len(nodes_2)<len(g.nodes):
    ...
    nx.draw_networkx_nodes(g,pos,nodelist=nodes_2,edgecolors=edgecolors2,linewidths=edge_width,node_color = cr2,node_shape='s',node_size=wt2) # first neighbor
     
    # pos_nodes = nudge(pos, 0, 0.065)                             # shift the layout
...
def plot_the_top_ranked_genes_in_community(df,gm,gdsc_df1,dep_df,t_type,t_subtype,comm_id,fig_dir,top_k=60,
    ...
    nx.draw_networkx_nodes(g,pos,nodelist=nodes_2,edgecolors=edgecolors2,linewidths=edge_width,node_color = cr2,node_shape='s',node_size=node_size,cmap = cmap,vmin=vmin,vmax=vmax) # first neighbor
